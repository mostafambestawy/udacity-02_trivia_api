...
def create_app(test_config=None):
  ...
  def index():
    qeustions=Question.query.order_by(func.random()).paginate(1,per_page=QUESTIONS_PER_PAGE)
    qs=[]
    current_cats=[]
  ...
  def get_questions():
    try:
      ...
      qeustions=Question.query.order_by(func.random()).paginate(page,per_page=QUESTIONS_PER_PAGE)
      qs=[]
      current_cats=[]
      for i in qeustions.items:
        qs.append(i.format())
  ...
  def get_by_cat(id):
    page=request.headers.get('page',1,type=int)
    qeustions=Question.query.filter(Question.category==id).paginate(page,per_page=QUESTIONS_PER_PAGE)
    qs=[]
    current_cats=[]
    for i in qeustions.items:
      qs.append(i.format())
  ...
  def search_question():
    try:
      ...
      qeustions=Question.query.filter((func.lower(Question.question)).like('%'+searchTerm+'%')).all()
      categories=Category.query.all()
      qs=[]
      current_cats=[]
      for i in qeustions:
  ...
  def play_quizzes():
    try:
      ...
      if quiz_category['id'] !=0:
        question=Question.query.filter(Question.id.notin_(previous_questions)).filter(Question.category==quiz_category['id']).order_by(func.random()).first()
      else:
